...
- `pacific-linux-disaster-info` — the app itself (GTK3 via PyGObject, same
  stack as `welcome-app/` for the same reason: matches XFCE's own native
  GTK3 apps)
- `pacific-linux-disaster-info.desktop` — applications-menu entry
- `content/` — per-country JSON data (see `content/README.md` for the
  schema and how to add a country)

UI chrome strings are gettext-wrapped under a separate `pacific-linux-disaster`
domain (not the welcome app's `pacific-linux` domain), so the catalogs stay
...
`.../usr/share/applications/`, and `.../usr/share/pacific-linux/disaster-info/`
